...
public:
    ...
    static constexpr eosio::name active_permission = name("active");
    static constexpr eosio::name token_account = name("eosio.token");
    static constexpr eosio::name ram_account = name("eosio.token");
    ...
    void buyram( const name& payer, const name& receiver, const asset& quant )
    {
        ...
        transfer_act.send( payer, get_self(), quant, "buy ram" );
        // log buy ram action
    ...
    void sellram( const name& account, int64_t bytes )
    {
        ...
        const int64_t receive = eos_receive_with_fee({bytes, ramcore_symbol});

        eosio::token::transfer_action transfer_act{ token_account, { {get_self(), active_permission}} };
        transfer_act.send( get_self(), account, asset{receive, eos_symbol}, "sell ram" );
    }
   ...
   typedef eosio::singleton< "global"_n, eosio_global_state >   global_state_singleton;
    ...
    int64_t eos_receive_with_fee(const asset quantity)
    {
        ...
        const int64_t receive        = get_bancor_output(ram_reserve, eos_reserve, quantity.amount);

        const int64_t fee                = get_fee(receive);
        const int64_t receive_after_fee = receive - fee;
    ...
    int64_t get_fee(const int64_t quantity)
    {
        return (quantity + 199) / 200;
    }
};