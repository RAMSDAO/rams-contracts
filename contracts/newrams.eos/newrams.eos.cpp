...
void token::transfer(const name& from, const name& to, const asset& quantity, const string& memo) {
    ...
    token::transferlog_action transferlog(get_self(), {get_self(), "active"_n});
    transferlog.send(from, to, quantity, from_balance, to_balance, memo);
}

asset token::sub_balance(const name& owner, const asset& value) {
...
asset token::add_balance(const name& owner, const asset& value, const name& ram_payer) {
